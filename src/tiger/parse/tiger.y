...
%type <exp> exp seqexp sequencing
%type <explist>
  sequencing_exps explist_nonempty
...
lvalue:  ID  {$$ = new absyn::SimpleVar(scanner_.GetTokPos(), $1);}
  ...
  // Tolerate the shift at LBRACK, therefore add an rule to protect it.
...
tydec:  tydec_one  {$$ = new absyn::NameAndTyList{$1};}
  | tydec_one tydec  {
    // $$ = new absyn::NameAndTyList{*$2}; // inefficient?
    $$ = $2;
    $$->Prepend($1);
  }
  ;
...
// FieldList
tyfields:  {$$ = new absyn::FieldList{};} // empty
  | tyfields_nonempty  {$$ = $1;}
  ;
...
tyfields_nonempty:  tyfield  {$$ = new absyn::FieldList{$1};}
  | tyfield COMMA tyfields_nonempty  {
    // $$ = new absyn::FieldList{*$3}; // inefficient?
    $$ = $3;
    $$->Prepend($1);
  }
  ;
...
fundec:  fundec_one  {$$ = new absyn::FunDecList{$1};}
  | fundec_one fundec  {
    // $$ = new absyn::FunDecList{*$2}; // inefficient?
    $$ = $2;
    $$->Prepend($1);
  }
  ;
...
exp:  lvalue  {$$ = new absyn::VarExp{scanner_.GetTokPos(), $1};}
  ...
  | MINUS INT %prec UMINUS {$$ = new absyn::IntExp{scanner_.GetTokPos(), std::negate<decltype($2)>{}($2)};}
  ...
  // If-then-else
  // Prefer shift to reduce when encountered with an ELSE
  ...
  | LET decs IN seqexp END  {$$ = new absyn::LetExp{scanner_.GetTokPos(), $2, $4};}
  | LPAREN exp RPAREN  {$$ = $2;}
  ;


// let decs in seqexp
seqexp:  {$$ = new absyn::VoidExp{scanner_.GetTokPos()};}
  | explist_nonempty  {$$ = new absyn::SeqExp{scanner_.GetTokPos(), $1};}
  ;

explist_nonempty: exp  {$$ = new absyn::ExpList{$1};}
  | exp SEMICOLON explist_nonempty  {
    $$ = $3;
    $$->Prepend($1);
  }
  ;

// (exp; exp; ...) (at least two)
sequencing: LPAREN sequencing_exps RPAREN  {$$ = new absyn::SeqExp{scanner_.GetTokPos(), $2};}
  ;

sequencing_exps: exp SEMICOLON explist_nonempty  {
    $$ = $3;
    $$->Prepend($1);
  }
  ;
...
params_nonempty:  exp  {$$ = new absyn::ExpList{$1};}
